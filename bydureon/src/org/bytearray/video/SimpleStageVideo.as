...
package org.bytearray.video
{
	...
	{		
		 ...
		 * 
		 */		
		public function SimpleStageVideo(stage:Stage, rect:Rectangle)
		{
			this.stage = stage;
			
			if (rect == null) 
			{
				_videoRect = new Rectangle(0, 0, stage.stageWidth, stage.stageHeight);
				fillScreen = true;
			}
			else 
			{
		...
		{	
			trace('video rect:', _videoRect);
			
			_videoRect.width = width, _videoRect.height = height;
			
			if ( _stageVideoInUse )	
			{
				trace('viewport:', _sv.viewPort);
				_sv.viewPort = getVideoRect(1024, 580);
			}				
			else 
			{
		...
		private function onRenderState(event:Event):void
		{
			...
			resize(_videoRect.width, _videoRect.height);
		} 
		...
		{	
			...
			if (fillScreen)
			{
				var scaling:Number = Math.min ( _videoRect.width / videoWidth, _videoRect.height / videoHeight );
				videoWidth *= scaling, videoHeight *= scaling;
			}			
			
			var posX:Number = fillScreen ? stage.stageWidth - videoWidth >> 1 : _videoRect.x;
			var posY:Number = fillScreen ? stage.stageHeight - videoHeight >> 1 : _videoRect.y;
