package com.wehaverhythm.utils
{
	public class ArrayUtils
	{
		...
		public static function shuffleArray(orig:Array):Array
		{
			var shuffledLetters:Array = new Array(orig.length);
			
			var randomPos:int = 0;
			for (var i:int = 0; i < orig.length; i++)
			{
				randomPos = int(Math.random() * orig.length);
				shuffledLetters[randomPos] = orig[i];
			}
